...
write.table(predictor_list,
            file=paste(OUT_DIR,"Subchallenge3_burkhajo_Time24_Predictors.csv",sep=""),
            sep=",",
            row.names=FALSE,
            quote=FALSE)

# select each subject's latest microarray (t<=24h) and power log score for svm training
latest_exprs <- snm_eset[top_2000$probesetid, ] %>%
  exprs() %>%
  as.data.frame() %>%
  dplyr::select(match(train_earliest_latest %>%
                        dplyr::filter(as.character(MICROARRAY_TIME) == "late") %>%
                        dplyr::mutate(CEL = as.character(CEL)) %>%
                        dplyr::select(CEL) %>% .[,1],names(.)))

latest_scores <- train_earliest_latest[match(latest_exprs %>%
...
latest_training_data <- latest_exprs %>%
  t() %>%
  as.data.frame() %>%
...
# select each subject's latest microarray
test_latest <- test_clinicl_df %>%
  dplyr::group_by(SUBJECTID) %>%
  dplyr::arrange(TIMEHOURS) %>%
  dplyr::slice(n()) %>%
  dplyr::ungroup() %>%
...
test_latest_exprs <- test_rma_eset[top_2000$probesetid, ] %>%
  exprs() %>%
  as.data.frame() %>%
  dplyr::select(match(test_latest %>%
                        dplyr::mutate(CEL = as.character(CEL)) %>%
                        dplyr::select(CEL) %>% .[,1],names(.)))

test_latest_subjects <- test_latest[match(test_latest_exprs %>%
                                                 colnames(),test_latest %>%
                                                 dplyr::mutate(CEL = as.character(CEL)) %>%
                                                 dplyr::select(CEL) %>% .[,1]),] %>%
                                              dplyr::select(CEL, SUBJECTID)

test_latest_data <- test_latest_exprs %>%
  t() %>%
  as.data.frame() %>%
  dplyr::add_rownames(var = "CEL") %>%
  dplyr::full_join(test_latest_subjects,by="CEL")
...
leaderboard_submission <- data.frame(
  SUBJECTID = test_latest_data$SUBJECTID,
  LOGSYMPTSCORE_SC3 = test_predictions) %>%
  dplyr::arrange(SUBJECTID)

write.table(leaderboard_submission,
            file="/home/burkhart/Software/TSAR/src/Subchallenge3_burkhajo_Time24_LeaderboardPredictions.csv",
            sep=",",
            row.names=FALSE,
            quote=FALSE)
